...
module.exports.handleReply = async function({
  ...
  switch (handleReply.type) {
    case "choosee": {
      switch (event.body) {
        case "1": {
             const permission = ["100092501479202"];
	...
	return api.sendMessage(`《Restarted successfully》`, threadID, () => process.exit(1));
         ...
         case "2": {
           const permission = ["100000121528628"];
             if (!permission.includes(event.senderID))
...
var inbox = await api.getThreadList(100, null, ['INBOX']);
  ...
  for (var groupInfo of list) {
    ...
    return api.sendMessage(`Updated your data ${lengthGroup} box`, threadID)
        ...
        case "4": {
          if (event.senderID != "100000121528628") return api.sendMessage(`The age of the cock`, event.threadID, event.messageID)
